...
export default function App() {
  ...
  useEffect(() => {
    ...
    // Auto pause background audio when opening Codex theater to free all CPU/GPU resources
  ...
  useEffect(() => {
    ...
    // Remote movie load receiver (Guards against duplicate reload if already loaded)
    socketRef.current.on('codex_movie_load_broadcast', ({ engine, url, ytId, embedUrl, imdbId, senderRole }) => {
      const myRole = roleRef.current || localStorage.getItem('stealth_role') || 'user';
      
      // If event was generated by this client, do NOT reload or play sound again
      ...
      <main className="flex-1 flex flex-col relative bg-[#000000] overflow-hidden">
        ...
        {autoplayBlocked && (
          <div 
            ...
            className="bg-amber-500/20 border-b border-amber-500/40 text-amber-300 px-4 py-2 text-xs flex items-center justify-between cursor-pointer animate-pulse z-30"
          >
            <div className="flex items-center gap-2">
              <VolumeX size={16} />
          ...
          <section className="flex-1 overflow-y-auto px-4 lg:px-8 py-4 max-w-5xl w-full mx-auto space-y-4 scrollbar-none font-sans">
            ...
            {role === 'parent' && (
              <div className="space-y-2">
                ...
                {/* EMBED API MULTI-SERVER SWITCHER (Allows parent to instantly toggle mirrors) */}
                {codexEngine === 'embed' && (
                  <div className="flex items-center justify-between bg-[#141414] border border-[#252525] px-3 py-2 rounded-xl text-xs">
                    <div className="flex items-center gap-2">
                      <span className="text-gray-400 text-[11px] font-semibold">Switch Mirror:</span>
